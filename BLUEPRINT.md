...
**DO NOT BUILD UNLESS TIME REMAINS** (only after Fourth is fully done): team capability graph, multi-role support beyond Data Analyst, real auth/login. `freshness_engine`, a recruiter dashboard, candidate comparison, and a skill evolution timeline were all explicitly requested and built anyway (see `app/services/freshness_engine.py`, `app/services/recruiter_dashboard.py`, `app/services/timeline_engine.py`, `frontend/app/recruiter/page.tsx`) — the recruiter dashboard is DB-backed only (no session-state fallback exists for cross-candidate data) and computed with real persisted claims (`candidate_claims`, migration 003) but no GitHub evidence, since the latter is still never stored in the DB; candidate comparison is the same dashboard payload (`CandidateSummary.skill_breakdown`) selected client-side, not a second endpoint; the timeline is a candidate-facing "Skill Evolution" section on the Readiness screen (`readiness/page.tsx`), reshaping `submission_history` into a per-attempt score history instead of readiness_engine's "latest wins" collapse. See CLAUDE.md.
...
34. [ ] Deploy backend (Render/Railway) and frontend (Vercel); verify CORS and env vars in production. (Deployment config exists and has been verified — `render.yaml`/env vars match `.env.example`/`config.py` with no drift, the app boots clean on a Render-shaped `$PORT` with zero secrets set, CORS was exercised live against production-shaped origins (allowed origin echoed back, stranger origin gets a 400), and a production-shaped frontend build correctly bakes in `NEXT_PUBLIC_API_BASE_URL` — see CLAUDE.md "Deployment config verification". A full step-by-step runbook exists at `docs/deploy_runbook.md` for whoever can actually reach Render/Vercel/Supabase — every session that's attempted this so far had two independent blockers, not one: no credentials for those services *and*, separately, a sandboxed network policy that returns a `403` on any connection attempt to `render.com`/`vercel.com`/`supabase.com` (confirmed directly, not assumed). Credentials alone won't unblock this from such an environment — it needs to run somewhere the network policy actually allows reaching those hosts, per the runbook's own note. This item stays open until that happens.)
...
37. [x] **Bonus, beyond the original 36-item scope.** Persist candidate claims for real (`candidate_claims` table, migration 003, `app/db/claims.py`), closing the `POST /candidates/claims` stub that item 11 originally shipped with and wiring `recruiter_dashboard.py`'s claim-alignment bonus to read them instead of always passing `claims=[]`.
