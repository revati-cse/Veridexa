...
**DO NOT BUILD UNLESS TIME REMAINS** (only after Fourth is fully done): team capability graph, multi-role support beyond Data Analyst, real auth/login. `freshness_engine`, a recruiter dashboard, candidate comparison, and a skill evolution timeline were all explicitly requested and built anyway (see `app/services/freshness_engine.py`, `app/services/recruiter_dashboard.py`, `app/services/timeline_engine.py`, `frontend/app/recruiter/page.tsx`) — the recruiter dashboard is DB-backed only (no session-state fallback exists for cross-candidate data) and computed with no persisted claims or GitHub evidence, since neither is stored in the DB yet; candidate comparison is the same dashboard payload (`CandidateSummary.skill_breakdown`) selected client-side, not a second endpoint; the timeline is a candidate-facing "Skill Evolution" section on the Readiness screen (`readiness/page.tsx`), reshaping `submission_history` into a per-attempt score history instead of readiness_engine's "latest wins" collapse. See CLAUDE.md.
